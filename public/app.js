...
async function loadServer(){if(!state.connected)return;try{const data=await request('/api/v2/server?type=session');state.server=data;$('#sessionRaw').textContent=pretty(data);const o=(data&&typeof data==='object')?data:{};const players=first(o,['player_count','playerCount','PlayerCount','current_players','currentPlayers','CurrentPlayers'],null);const max=first(o,['max_player_count','maxPlayerCount','max_players','maxPlayers','MaxPlayers','slots','Slots'],null);const playerNumber=Number(players),maxNumber=Number(max);if(players!==null&&Number.isFinite(playerNumber)){$('#statPlayers').textContent=max!==null&&Number.isFinite(maxNumber)?`${playerNumber}/${maxNumber}`:String(playerNumber);$('#statPlayerSub').textContent='live server population'}
    const map=first(o,['map','Map','map_name','mapName','MapName','map_id','mapId','current_map','currentMap','CurrentMap']);const next=first(o,['next_map','nextMap','NextMap','next_map_name','nextMapName','next_map_id','nextMapId','MapNext']);const time=first(o,['remaining_time','remainingTime','RemainingTime','remaining_match_time','remainingMatchTime','time_remaining','timeRemaining','TimeRemaining','remaining']);$('#statMap').textContent=String(map);$('#statNextMap').textContent=String(next);$('#statTime').textContent=String(time)
